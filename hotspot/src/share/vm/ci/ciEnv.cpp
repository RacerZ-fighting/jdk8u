...
ciInstance *ciEnv::ArrayIndexOutOfBoundsException_instance()
{
  ...
  {
    _ArrayIndexOutOfBoundsException_instance = get_or_create_exception(_ArrayIndexOutOfBoundsException_handle,
...
ciInstance *ciEnv::ArrayStoreException_instance()
{
  ...
  {
    _ArrayStoreException_instance = get_or_create_exception(_ArrayStoreException_handle,
...
ciInstance *ciEnv::ClassCastException_instance()
{
  ...
  {
    _ClassCastException_instance = get_or_create_exception(_ClassCastException_handle,
                                          ...
                                          ciInstanceKlass *accessor)
{
  ...
  else
  {
    ...
    if (cpool->has_preresolution() || (holder == ciEnv::MethodHandle_klass() &&
...
void ciEnv::validate_compile_task_dependencies(ciMethod *target)
{
  ...
  {
    if (deps.is_klass_type())
      continue; // skip klass dependencies
  ...
  {
    if (!deps.is_klass_type())
      continue; // skip non-klass dependencies
    Klass *witness = deps.check_dependency();
    if (witness != NULL)
    {
      klass_violations++;
...
// ------------------------------------------------------------------
// zip
// void test_insert(uintptr_t x, int index)
// {
//   if (test_h[TEST_N - 1] == 0)
//   {
//     std::memset(test_h, -1, sizeof test_h);
//     test_idx = 0;
//   }
//   test_e[test_idx] = x;
//   test_ne[test_idx] = test_h[index];
//   test_h[index] = test_idx++;
// }
