...
void VM_RedefineClasses::doit_epilogue()
{
  ...
  if (RC_TRACE_ENABLED(0x00000004))
  {
    ...
    RC_TRACE(0x00000004, ("vm_op: all=" UINT64_FORMAT
                          "  prologue=" UINT64_FORMAT "  doit=" UINT64_FORMAT,
                          all_time,
                                      ...
                                      TRAPS)
{
  ...
  case JVM_CONSTANT_Class:
  {
    ...
    (*merge_cp_p)->unresolved_klass_at_put(*merge_cp_length_p, scratch_cp->klass_name_at(scratch_i));
  ...
  case JVM_CONSTANT_NameAndType:
  {
    ...
    (*merge_cp_p)->name_and_type_at_put(*merge_cp_length_p, new_name_ref_i, new_signature_ref_i);
    if (scratch_i != *merge_cp_length_p)
    {
      // The new entry in *merge_cp_p is at a different index than
      // the new entry in scratch_cp so we need to map the index values.
  ...
  case JVM_CONSTANT_Methodref:
  {
    ...
    case JVM_CONSTANT_Fieldref:
      entry_name = "Fieldref";
      (*merge_cp_p)->field_at_put(*merge_cp_length_p, new_klass_ref_i, new_name_and_type_ref_i);
      break;
    case JVM_CONSTANT_InterfaceMethodref:
      entry_name = "IFMethodref";
      (*merge_cp_p)->interface_method_at_put(*merge_cp_length_p, new_klass_ref_i, new_name_and_type_ref_i);
      break;
    case JVM_CONSTANT_Methodref:
      entry_name = "Methodref";
      (*merge_cp_p)->method_at_put(*merge_cp_length_p, new_klass_ref_i, new_name_and_type_ref_i);
      break;
    default:
  ...
  for (int i = 0; i < _class_count; i++)
  {
    ...
    if (!the_class->is_linked())
    {
      ...
      if (HAS_PENDING_EXCEPTION)
      {
        Symbol *ex_name = PENDING_EXCEPTION->klass()->name();
        // RC_TRACE_WITH_THREAD macro has an embedded ResourceMark
        RC_TRACE_WITH_THREAD(0x00000002, THREAD, ("link_class exception: '%s'", ex_name->as_C_string()));
        CLEAR_PENDING_EXCEPTION;
...

  {
    ...
    for (old_i = 1; old_i < old_cp->length(); old_i++)
    {
      ...
      case JVM_CONSTANT_UnresolvedClass:
        ...
        (*merge_cp_p)->unresolved_klass_at_put(old_i, old_cp->klass_name_at(old_i));
        break;
...
public:
  MergeCPCleaner(ClassLoaderData *loader_data, ConstantPool *merge_cp) : _loader_data(loader_data), _cp(merge_cp), _scratch_cp(NULL) {}
  ~MergeCPCleaner()
  {
    ...
    TRAPS)
{
  ...
  int merge_cp_length = the_class->constants()->length() + scratch_class->constants()->length();
  ...
  // an error.
  if (merge_cp_length != the_class->constants()->length() + scratch_class->constants()->length())
  {
    return JVMTI_ERROR_INTERNAL;
  }
    ...
    instanceKlassHandle scratch_class, TRAPS)
{
  ...
  for (int i = 0; i < scratch_class->methods()->length(); i++)
  {
    ...
    if (method_parameter_annotations == NULL || method_parameter_annotations->length() == 0)
    {
    ...
    instanceKlassHandle scratch_class, TRAPS)
{
  ...
  for (int i = 0; i < scratch_class->methods()->length(); i++)
  {
    ...
    if (method_default_annotations == NULL || method_default_annotations->length() == 0)
    {
...
void VM_RedefineClasses::AdjustCpoolCacheAndVtable::do_klass(Klass *k)
{
  ...
  else if (k->oop_is_instance())
  {
    ...
    if (ik->vtable_length() > 0 && (_the_class_oop->is_interface() || _the_class_oop == SystemDictionary::misc_Unsafe_klass() || ik->is_subtype_of(_the_class_oop)))
    {
    ...
    if (ik->itable_length() > 0 && (_the_class_oop->is_interface() || _the_class_oop == SystemDictionary::misc_Unsafe_klass() || ik->is_subclass_of(_the_class_oop)))
    {
                                               ...
                                               Klass *scratch_class_oop, TRAPS)
{
  ...
  the_class->set_source_debug_extension(
      ...
      scratch_class->source_debug_extension() == NULL ? 0 : (int)strlen(scratch_class->source_debug_extension()));
