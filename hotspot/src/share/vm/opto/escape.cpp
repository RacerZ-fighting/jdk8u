...
ConnectionGraph::ConnectionGraph(Compile *C, PhaseIterGVN *igvn) : _nodes(C->comp_arena(), C->unique(), C->unique(), NULL),
  ...
  if (t->make_ptr() != NULL)
  {
    ...
    else
    {
      assert((ptnode_adr(adr->_idx) == NULL ||
              ptnode_adr(adr->_idx)->as_Field()->is_oop()),
             "sanity");
    }
#endif
...
void ConnectionGraph::add_node_to_connection_graph(Node *n, Unique_Node_List *delayed_worklist)
{
  ...
  default:; // Do nothing for nodes not related to EA.
  }
  return;
}
  ...
  for (UseIterator i(jobj); i.has_next(); i.next())
  {
    ...
    if (use->is_Field())
    {
      ...
      assert(field->is_oop() && field->scalar_replaceable() &&
                 field->fields_escape_state() == PointsToNode::NoEscape,
             "sanity");
...
Node *ConnectionGraph::get_addp_base(Node *addp)
{
  ...
  if (base->uncast()->is_top())
  { // The AddP case #3 and #6.
    ...
    if (abase != base)
    {
      ...
      else
      {
        ...
        assert(adr->is_AddP() && atype != NULL &&
                   atype->instance_id() == inst_id,
               "array's element offset should be processed first");
#endif
      }
...
void ConnectionGraph::split_unique_types(GrowableArray<Node *> &alloc_worklist)
{
  ...
  while (alloc_worklist.length() != 0)
  {
    ...
    if (n->is_Call())
    {
      ...
      n = alloc->result_cast();
      if (n == NULL)
      { // No uses except Initialize node
        if (alloc->is_Allocate())
        {
      ...
      if (alloc->is_Allocate() && n->as_Type()->type() == TypeInstPtr::NOTNULL && (alloc->is_AllocateArray() || igvn->type(alloc->in(AllocateNode::KlassNode)) != TypeKlassPtr::OBJECT))
      {
        Node *cast2 = NULL;
        for (DUIterator_Fast imax, i = n->fast_outs(imax); i < imax; i++)
        {
    ...
    else if (n->is_AddP())
    {
      ...
      if (!split_AddP(n, base))
        continue; // wrong type from dead path
    }
    else if (n->is_Phi() ||
             n->is_CheckCastPP() ||
             n->is_EncodeP() ||
    ...
    for (DUIterator_Fast imax, i = n->fast_outs(imax); i < imax; i++)
    {
      ...
      {
        if (use->in(TypeFunc::Memory) == n)
        { // Ignore precedent edge
          memnode_worklist.append_if_missing(use);
        }
      }
      else if (use->is_AddP() && use->outcnt() > 0)
      { // No dead nodes
