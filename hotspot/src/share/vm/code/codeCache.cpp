...
{
  assert_locked_or_safepoint(CodeCache_lock);
  while (cb != NULL && (!cb->is_alive() || !cb->is_nmethod()))
    cb = next(cb);
  return (nmethod *)cb;
}
...
void CodeCache::scavenge_root_nmethods_do(CodeBlobToOopClosure *f)
{
  ...
  while (cur != NULL)
  {
    ...
    if (TraceScavenge)
    {
      cur->print_on(tty, is_live ? "scavenge root" : "dead scavenge root");
      tty->cr();
    }
#endif // PRODUCT
    if (is_live)
...
void CodeCache::unlink_scavenge_root_nmethod(nmethod *nm, nmethod *prev)
{
  ...
  assert((prev == NULL && scavenge_root_nmethods() == nm) ||
             (prev != NULL && prev->scavenge_root_link() == nm),
         "precondition");
...
void CodeCache::print_summary(outputStream *st, bool detailed)
{
  ...
  if (detailed)
  {
    ...
    st->print_cr(" compilation: %s", CompileBroker::should_compile_new_jobs() ? "enabled" : Arguments::mode() == Arguments::_int ? "disabled (interpreter mode)"
